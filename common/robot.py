...
class robot:
    ...
    def move_2(self, turning, distance, tolerance = 0.001, max_turning_angle = pi):
        """This function turns the robot and then moves it forward."""
        # apply noise, this doesn't change anything if turning_noise
        # and distance_noise are zero.
        turning = random.gauss(turning, self.turning_noise)
        ...
        res.turning = self.turning  # only applies to target robots who constantly move in a circle
        res.distance = self.distance # only applies to target bot, who always moves at same speed.
        res.turning_noise = self.turning_noise    
        res.distance_noise = self.distance_noise    
        res.measurement_noise =self.measurement_noise 

        # Execute motion
        res.heading += turning
        res.heading = angle_trunc(res.heading)
        res.x += distance * cos(res.heading)
        res.y += distance * sin(res.heading)
        return res
