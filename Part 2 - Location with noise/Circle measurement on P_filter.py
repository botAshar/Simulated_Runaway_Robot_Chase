...
import time as t
import numpy as np
global p_list,num5,cond,last,cond4,predictor,num2
num2=0
cond=False
cond4=True
...
def cir_transform(x_measured,y_measured,x_center,y_center,radius):
    ...
    m = (x_measured-y_center)/(y_measured-x_center)
...
def next_pos(measurement, OTHER = None):
    ...
    cond3=False
    global last,cond,num5,cond4,predictor,num_list,num2,are_transformed
    ...
    if num5<=1:
        ...
        if not are_transformed:
            for i in range(len(OTHER)):
                #print(type(OTHER[i]))
                OTHER[i][0],OTHER[i][1]=cir_transform(OTHER[i][0],OTHER[i][1],mx_list[-1],my_list[-1],radius_list[-1])
            are_transformed=True
        x,y=cir_transform(measurement[0],measurement[1],mx_list[-1],my_list[-1],radius_list[-1])
        num2+=1
        OTHER.append([x,y])
        x_list.append(measurement[0])
        y_list.append(measurement[1])
